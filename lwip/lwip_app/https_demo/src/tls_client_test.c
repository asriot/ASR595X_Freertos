...
#define HTTPS_TLS_RECV_DATA_LEN 1024
#define HTTPS_TLS_VRIFY_RESULT_LEN 512
void https_tls_demo()
{
    int ret = 1, len;
    ...
    buf = (char *)lega_rtos_malloc(HTTPS_TLS_RECV_DATA_LEN);
    if (buf  == NULL)
    {
    ...
    {
        char *vrfy_buf;
        vrfy_buf = (char *)lega_rtos_malloc(HTTPS_TLS_VRIFY_RESULT_LEN);
        if (vrfy_buf  == NULL)
        {
            printf("Malloc memory failed \r\n");
            goto exit;
    ...
    do
    {
        len = HTTPS_TLS_RECV_DATA_LEN - 1;
        memset( buf, 0, HTTPS_TLS_RECV_DATA_LEN);
