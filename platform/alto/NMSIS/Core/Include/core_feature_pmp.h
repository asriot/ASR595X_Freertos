 ...
 * \details Configure a PMP region with the given index, configuration and address.
 * \param [in]    idx      PMP region index(0-15)
 * \param [in]    pmpxcfg  PMPxCFG register value to set, which is combination of
 *                         PMP_R/PMP_W/PMP_X/PMP_L, but PMP_A* will be ignored
 * \param [in]    addr_base  base address aligned power-of-two
 * \param [in]    addr_len_power  the number of power-of-two for address length
 * \return        -1 means invalid input parameter. 0 means successful.
 */
__STATIC_INLINE int __config_PMPx(uint32_t idx, uint8_t pmpxcfg, rv_csr_t addr_base, uint8_t addr_len_power)
{
    rv_csr_t low_bits;
    if(idx > __PMP_ENTRY_NUM || addr_len_power < 2 || addr_len_power > (sizeof(rv_csr_t)*8))
        return -1;

    low_bits = (0x1<<addr_len_power) - 1;
    if(addr_base & low_bits)
        return -1;

    pmpxcfg = pmpxcfg & 0xe7;
    if(addr_len_power == 2)
    {
        pmpxcfg |= 0x10;   //NA4
    }
    else
    {
        pmpxcfg |= 0x18;   //NAPOT
        addr_base &= (~low_bits);
        addr_base |= (low_bits>>1);
    }

    __set_PMPADDRx(idx, addr_base>>2);
