 ...
 */
static uint64_t g_counts_for_one_tick = 0;
__STATIC_INLINE uint32_t SysTick_Config(uint64_t ticks)
{
    SysTimer_SetLoadValue(0);
    SysTimer_SetCompareValue(ticks);
    SysTimer_SetControlValue(SysTimer_MTIMECTL_CMPCLREN_Msk|SysTimer_MTIMECTL_CLKSRC_Msk);
    ECLIC_SetTrigIRQ(SysTimer_IRQn,ECLIC_POSTIVE_EDGE_TRIGGER);
    ECLIC_SetShvIRQ(SysTimer_IRQn, ECLIC_NON_VECTOR_INTERRUPT);
    ECLIC_SetLevelIRQ(SysTimer_IRQn, 1);
    ECLIC_EnableIRQ(SysTimer_IRQn);

    g_counts_for_one_tick = ticks;
    return (0UL);
}

__STATIC_INLINE uint32_t SysTick_ReConfig(uint64_t ticks)
{
    uint64_t value = SysTimer_GetLoadValue();
    uint64_t cmp_value  = SysTimer_GetCompareValue();
    value = value * ((float)ticks / g_counts_for_one_tick);
    cmp_value = cmp_value * ((float)ticks / g_counts_for_one_tick);

    SysTimer_SetLoadValue(value);
    SysTimer_SetCompareValue(cmp_value);
    SysTimer_SetControlValue(SysTimer_MTIMECTL_CMPCLREN_Msk|SysTimer_MTIMECTL_CLKSRC_Msk);
    ECLIC_SetTrigIRQ(SysTimer_IRQn,ECLIC_POSTIVE_EDGE_TRIGGER);
    ECLIC_SetShvIRQ(SysTimer_IRQn, ECLIC_NON_VECTOR_INTERRUPT);
    ECLIC_SetLevelIRQ(SysTimer_IRQn, 1);
