...
typedef void (*alto_i2c_slv_tx_callback_func)(void);
typedef void (*alto_i2c_slv_rx_callback_func)(uint8_t);
typedef struct
{
    alto_i2c_slv_tx_callback_func tx_func;
    alto_i2c_slv_rx_callback_func rx_func;
} alto_i2c_slv_callback_t;
...
typedef struct
{
    ...
    alto_i2c_config_t config;  /* i2c config */
    void        *priv;    /* priv data */
} alto_i2c_dev_t;

int32_t alto_i2c_init(alto_i2c_dev_t *i2c);
int32_t alto_i2c_master_send(alto_i2c_dev_t *i2c, uint16_t dev_addr, const uint8_t *data, uint16_t size, uint32_t timeout);
int32_t alto_i2c_master_recv(alto_i2c_dev_t *i2c, uint16_t dev_addr, uint8_t *data, uint16_t size, uint32_t timeout);
int8_t alto_i2c_master_repeated_write_read(I2C_TypeDef * I2Cx, uint8_t slave_addr,
                                            const uint8_t * pwdata, uint8_t * rdata,
                                            uint32_t wlen, uint32_t rlen);
int32_t alto_i2c_mem_write(alto_i2c_dev_t *i2c, uint16_t dev_addr, uint16_t mem_addr,
                           uint16_t mem_addr_size, const uint8_t *data, uint16_t len,
                           uint32_t timeout);

int32_t alto_i2c_mem_read(alto_i2c_dev_t *i2c, uint16_t dev_addr, uint16_t mem_addr,
                          uint16_t mem_addr_size, uint8_t *data, uint16_t len,
                          uint32_t timeout);
int32_t alto_i2c_finalize(alto_i2c_dev_t *i2c);

void alto_i2c_master_dma_send(uint8_t iic_idx,uint32_t *data,uint16_t len);
void alto_i2c_master_dma_recv(uint8_t iic_idx,uint32_t *data,uint16_t len);
