 ...
 *    Initial release
 */
#ifndef _ASR_FLASH_ALG_H_
#define _ASR_FLASH_ALG_H_
...
int  asr_flash_alg_init (void);
int  asr_flash_alg_erase(unsigned int cmd, unsigned long adr);
int  asr_flash_alg_programpage (unsigned long adr,   // Program Page Function
                      unsigned long sz,
                      unsigned char *buf);
int asr_flash_alg_is_wip (void);
#ifdef _SPI_FLASH_240MHz_
void asr_flash_alg_set_clk_240(unsigned char en_240m);
#endif
#if (defined(_SPI_FLASH_120MHz_) || defined(CFG_OPTIMIZE_CHANNEL_SENSITIVITY))
void asr_flash_alg_set_clk_120(unsigned char en_120m);
#endif

int asr_flash_alg_read_id(void);
int asr_flash_alg_read_uuid(unsigned int *uuid_buf, unsigned int buf_size);
void asr_flash_alg_dp(void);
void asr_flash_alg_dp_release(void);

#ifdef __cplusplus
}
#endif

#endif //_ASR_FLASH_ALG_H_
