 ...
 */
#include "asr_alto_ram_layout.h"
void open_clock_wifi_ble()
{
    uint32_t tmp_value;
    tmp_value = REG_RD(PERI_CLK_EN_REG0);
    REG_WR(PERI_CLK_EN_REG0, (tmp_value | (0x00FF8000))); //open clock of wifi and ble
}

#define WIFI_ADC_SHARED (32 * 1024)

uint32_t asr_ram_layout_init(Tcm_Config_Type tcm_config, Wifi_Ram_Config_Type wifi_config, Bt_Ram_Config_Type bt_config)
{
    ...
    switch(tcm_config) {
        case ITCM_DTCM_128_256:
            reg_tcm = 0;
            break;
        case ITCM_DTCM_384_0:
            reg_tcm = 1;
            break;
        default:
            return -1;
...
uint32_t asr_get_ram_layout(Ram_Layout_Type *ram_layout)
{
    ...
    if(reg_tcm == 0)
    {
        ...
        ram_layout->itcm_size = (128 * 1024);
        ram_layout->dtcm_addr = 0x20FC0000;
        ram_layout->dtcm_size = (256 * 1024);
    }
    else
    {
        ram_layout->itcm_addr = 0x00080000;
        ram_layout->itcm_size = (384 * 1024);
        ram_layout->dtcm_addr = 0x21000000;
        ram_layout->dtcm_size = (0 * 1024);
    }
