 ...
 */
//uint32_t SystemCoreClock = system_core_clk;  /* System Clock Frequency (Core Clock) */
...
void SystemCoreClockUpdate (void)            /* Get Core Clock Frequency */
{
     ...
     */
    //SystemCoreClock = system_core_clk;
}
...
void SystemInit(void)
{
  ...
  xHeapRegions[ 1 ].pucStartAddress = (uint8_t*)&sram_heap_start;
  xHeapRegions[ 1 ].xSizeInBytes = (size_t)&sram_heap_len;
  vPortDefineHeapRegions(xHeapRegions);
}

#else //MULTI_HEAP_REGIONS
...
#if defined(SYSTEM_COREDUMP) && !defined(ALIOS_SUPPORT)
extern void HardFault_Handler_C(unsigned int *hardfault_args, unsigned int mcause);
#endif
static void system_default_exception_handler(unsigned long mcause, unsigned long sp)
{
    ...
    printf("MCAUSE: 0x%lx SP:0x%lx \r\n", mcause,sp);
    printf("EX code:%ld,MDCAUSE: 0x%lx\r\n", EXCn,__RV_CSR_READ(CSR_MDCAUSE));
    printf("pmp status:0x%lx 0x%lx 0x%lx 0x%lx\r\n", __RV_CSR_READ(CSR_PMPCFG0), __RV_CSR_READ(CSR_PMPCFG1), __RV_CSR_READ(CSR_PMPCFG2), __RV_CSR_READ(CSR_PMPCFG3));
    printf("MEPC  : 0x%lx\r\n", __RV_CSR_READ(CSR_MEPC));
    printf("MTVAL : 0x%lx\r\n", __RV_CSR_READ(CSR_MBADADDR));
#if defined(DEBUG_BACKTRACE) && !defined(ALIOS_SUPPORT)
    debug_backtrace_exception();
#endif
#if defined(SYSTEM_COREDUMP) && !defined(ALIOS_SUPPORT)
    HardFault_Handler_C((unsigned int *)sp, (unsigned int)mcause);
#endif
    while(1);
}
...
uint32_t core_exception_handler(unsigned long mcause, unsigned long sp)
{
    ...
    if ((EXCn < MAX_SYSTEM_EXCEPTION_NUM) && (EXCn >= 0)) {
        exc_handler = (EXC_HANDLER)system_default_exception_handler;//SystemExceptionHandlers[EXCn];
    } else if (EXCn == NMI_EXCn) {
